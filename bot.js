...
async function whatsAsena () {
    ...
    setInterval(async () => { 
        if (config.AUTOBİO) {
            ...
            var utch = new Date().toLocaleDateString(LK, get_localized_date)
...
//Bot şifre sistemi
    conn.on('open', async () => {
        ...
        console.log(
         ...
         console.log(
            chalk.red.bold('Şifreyi doğru yazdığınızdan emin olun'));
         throw new Error("Şifre hatalı ⚠⚠ ");         
         return; //created by Zlose
         }
...
*/
                        ...
                        try {
                            ...
                            if (config.NOLOG === 'off') {
                                ...
                                await conn.sendMessage(conn.user.jid, '*~_________~ Zlose dev ~______~*' +
                                    '\n*🌀 Silivri soğuktur yiğen*' +
                                    '\n\n*⚠️ ' + error + '*\n'
                                    , MessageType.text);
                            }
                        }
