...
Asena.addCommand({pattern: 'install?(.*)', fromMe: true, desc: Lang.INSTALL_DESC, warn: Lang.WARN}, (async (message, match) => {
    ...
    if (response.statusCode == 200) {
        ...
        fs.writeFileSync('/root/asena2/plugins/' + plugin_name + '.js', response.body);
        try {
            require('/root/asena2/plugins/' + plugin_name);
        } catch (e) {
            fs.unlinkSync('/root/asena2/plugins/' + plugin_name + '.js')
...
Asena.addCommand({pattern: 'remove(?: |$)(.*)', fromMe: true, desc: Lang.REMOVE_DESC}, (async (message, match) => {
    ...
    if (plugin.length < 1) {
        ...
        delete require.cache[require.resolve('/root/asena2/plugins/' + name + '.js')]
        fs.unlinkSync('/root/asena2/plugins/' + name + '.js');
